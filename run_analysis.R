...
# free
rm(subjectTrain, subjectTest, xTrain, xTest, yTrain, yTest, subjectData, xData, yData)

## 2. Extracts only the measurements on the mean and standard deviation for each measurement.
data <- data[, grepl("SubjectId|Activity|mean\\(\\)|std\\(\\)", names(data))]

## 3. Uses descriptive activity names to name the activities in the data set
activityNames$V2 <- sapply(activityNames$V2, function(x) {paste(sapply(strsplit(tolower(x), "_"), capitalize), collapse=" ")}) 
